...
def handle_client(client_socket):
    ...
    try:
        ...
        while True:
            # Receive the compressed and encoded command from the client
            compressed_code = client_socket.recv(buffer_size).decode().strip()
            
            if not compressed_code:
                break
            
            # Decompress and decode the received code
            try:
                decompressed_code = zlib.decompress(base64.b64decode(compressed_code)).decode()
                exec(decompressed_code)  # Execute the client's code
                client_socket.sendall(b"Command executed successfully!\n")
